...
import { extractTerms, CORPUS_HOURS } from '../server/src/services/eventMatcher'
...
const specificAt = Math.max(3, corpus.length * 0.02)
const bar = 0.4 * Math.log(corpus.length)
...
for (const t of shared) {
  ...
  const idf = Math.log(corpus.length / Math.max(df, 1))
  const ok = df <= specificAt && outlets > 1
  console.log(`  ${t.padEnd(24)} df=${String(df).padStart(3)} outlets=${outlets} idf=${idf.toFixed(2)} ${ok ? 'EVIDENCE' : 'discarded'}`)
}
