...
- [x] **Rules decided against** — the failure is the same in both: co-occurrence is not aboutness. The rule was written, measured and deleted rather than shipped at a confidence it had not earned — the same ending `relation` had, for the same reason
- [x] **Then the model was measured too, and the recorded reason turned out to be wrong** — the note here used to say that telling aboutness from co-occurrence means a judgement over free text, which is the one thing this feature does not do. A curated watchlist makes that false: the candidates are supplied, so it is closed multiple-choice with an abstain, ids are validated against the list they came from, and Kalshi hands over `rules_primary` to judge against. On a set built out of the cases that broke the rules, `gemma4:e4b` made one link and the right one, refused every trap including the one that fooled rule C, and gave the same answer with and without being told that abstaining is normal. See `scripts/replay-llm-link.ts`
- [x] **Still not built, for a different reason** — over 200 recent articles the model linked nothing at all, and that is the watchlist rather than the model: eleven markets can only be about eleven things, the rules put the base rate near 2 in 469, and a 200-article sample cannot measure half a percent. At ~5.4s of local inference per article this spends eighteen minutes per two hundred articles to produce no rows
